...
Take an **existing** Jira issue and ship it. Do not create tickets (use `write-jira-bug-ticket` for that). Always plan before coding; no approval gate — plan, then build.

Ticket descriptions in this project often follow the structure in `.cursor/skills/write-jira-bug-ticket/SKILL.md` (Summary, Steps to Reproduce, Expected/Actual, Environment, Evidence, Notes). Use those sections when present; adapt for New Feature / Story tickets that use acceptance criteria instead.
